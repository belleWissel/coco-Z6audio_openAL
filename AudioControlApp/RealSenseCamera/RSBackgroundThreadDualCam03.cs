...
{
    public abstract class RSBackgroundThreadDualCam03 : IDisposable
    {
        ...
        public static RSBackgroundThreadDualCam03 CreateForDevice(Sensor whichSensor01, string whichSeralNum01, int whichCamID01, Sensor whichSensor02, string whichSeralNum02, int whichCamID02)
            => new DeviceReadingLoop03(whichSensor01, whichSeralNum01, whichCamID01, whichSensor02, whichSeralNum02, whichCamID02);
        ...
        protected RSBackgroundThreadDualCam03() => backgroundThread = new Thread(BackgroundLoop) { IsBackground = true };

        public event EventHandler<CaptureReady01EventArgsDualCam03> CaptureReady01;
        public event EventHandler<CaptureReady02EventArgsDualCam03> CaptureReady02;
        public event EventHandler<FailedEventArgsDualCam03> Failed;
        ...
        private sealed class DeviceReadingLoop03 : RSBackgroundThreadDualCam03
        {
            ...
            public DeviceReadingLoop03(Sensor whichSensor01, string whichSerialNum01, int whichCamID01, Sensor whichSensor02, string whichSerialNum02, int whichCamID02)
            {
                    ...
                    while (isRunning) // begin endless loop
                    {
                        foreach (var camPipe in pipes) // when using pipe collection, do we waitForFrames or PollForFrames? (Poll)
                        {
                            if (camPipe.PollForFrames(out FrameSet frames))
                            {
                                ...
                                {
                                    CaptureReady01?.Invoke(this, new CaptureReady01EventArgsDualCam03(frames.DepthFrame));
                                }
                                else
                                {
    ...
    public sealed class FailedEventArgsDualCam03 : EventArgs
    {
        public FailedEventArgsDualCam03(Exception exception)
            => Exception = exception;

        public Exception Exception { get; }
    }

    public sealed class CaptureReady01EventArgsDualCam03 : EventArgs
    {
        public CaptureReady01EventArgsDualCam03(DepthFrame depth)
            => Capture = depth;

        public DepthFrame Capture { get; }
    }
    public sealed class CaptureReady02EventArgsDualCam03 : EventArgs
    {
        public CaptureReady02EventArgsDualCam03(DepthFrame depth)
            => Capture = depth;
