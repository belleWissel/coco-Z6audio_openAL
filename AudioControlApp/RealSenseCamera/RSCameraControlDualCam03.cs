...
{
    public class RSCameraControlDualCam03
    {
        private static int whichDepthW = 640; //480; //848;
        private static int whichDepthH = 360; //270; //480;
        ...
        public bool[] cameraIsActive = new bool[maxNumberOfDepthCameras];
        
        private bool[] overrideCamActivationFromConfig = new bool[maxNumberOfDepthCameras];
        public string[] listOfDevSN = new string[maxNumberOfDepthCameras];
        public string[] listOfDevFW = new string[maxNumberOfDepthCameras];
        ...
        private RSBackgroundThreadDualCam03 readingLoopDual;
        private RSBackgroundThreadSingleCam03 readingLoopSingle;
        ...
        public RSCameraControlDualCam03(int whichTransmitRate)
        {
            ...
            for (i = 0; i < maxNumberOfDepthCameras; ++i)
            {
                ...
                prevFPS[i] = 0;
                currentFPS[i] = 0;
                cameraIsActive[i] = false; // reset these status, wait for
                overrideCamActivationFromConfig[i] = false; // config file may request NOT to activate camera
            }
        ...
        {
            if (whichCam < maxNumberOfDepthCameras)
        ...
        public bool initDepthCameraDevice()
        {
            ...
            if (currentlyConnectedAndActiveDeviceCount>0) // there is a camera and it has been matched to device path
            {
                ...
                {
                    if (readingLoopDual == null)
                    {
                        readingLoopDual = RSBackgroundThreadDualCam03.CreateForDevice(depthCameraSensor0, listOfDevSN[0], 0, depthCameraSensor1, listOfDevSN[1], 1); // depthcameraSensor is assigned in the checkForConnected() function

                        readingLoopDual.CaptureReady01 += ReadingLoop_CaptureReady01;
                        readingLoopDual.CaptureReady02 += ReadingLoop_CaptureReady02;
                        readingLoopDual.Failed += ReadingLoop_Failed0;
                        System.Diagnostics.Debug.WriteLine("[RSCamCtrl] DUAL camera0 & camera1 ready");
                        valueToReturn = true;
                    }
                }
                else if (cameraIsActive[0])
                {
                    if (readingLoopSingle == null)
                    {
                        readingLoopSingle = RSBackgroundThreadSingleCam03.CreateForDevice(depthCameraSensor0, listOfDevSN[0], 0); // depthcameraSensor is assigned in the checkForConnected() function

                        readingLoopSingle.CaptureReady01 += ReadingLoop_CaptureReady;
                        //readingLoop1.CaptureReady02 += ReadingLoop_CaptureReady02;
                        readingLoopSingle.Failed += ReadingLoop_Failed;
                        System.Diagnostics.Debug.WriteLine("[RSCamCtrl] SINGLE camera0 ready");
                        valueToReturn = true;
                    }
                }
            }
        ...
        private void ReadingLoop_Failed0(object sender, FailedEventArgsDualCam03 e)
        {
            System.Diagnostics.Debug.WriteLine("[RSCamCtrl] ReadingLoop_Failed with [" + e.Exception.Message + "]");
            writeToEventLog("[RSCamCtrl] ReadingLoop_Failed with [" + e.Exception.Message + "]");
            readingLoopDual.resetAndRestartCameras();
        }
        
        private void ReadingLoop_Failed(object sender, FailedEventArgsSingleCam03 e)
        {
            System.Diagnostics.Debug.WriteLine("[RSCamCtrl] ReadingLoop_Failed with [" + e.Exception.Message + "]");
            writeToEventLog("[RSCamCtrl] ReadingLoop_Failed with [" + e.Exception.Message + "]");
            readingLoopSingle.resetAndRestartCameras();
        }

        private void ReadingLoop_CaptureReady(object sender, CaptureReady01EventArgsSingleCam03 e)
        {
            if (!sensorIsOn)
                return;
        ...
        private void ReadingLoop_CaptureReady01(object sender, CaptureReady01EventArgsDualCam03 e)
        {
            if (!sensorIsOn)
                return;
            /*
        ...
        private void ReadingLoop_CaptureReady02(object sender, CaptureReady02EventArgsDualCam03 e)
        {
            if (!sensorIsOn)
                return;
            /*
        ...
        // ***************************************
        string errorMessageFromLoop = "NA";
        ...
        private void processAndXmitData_Tick(object sender, EventArgs e)
        {
            if (!theWaitIsOver) // wait and initialize cameras some time after program launch...
            {
                ...
                if (updateDepthDataTimerWaitCounter > updateDepthDataTimerWaitCounterLimit)
                {
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine("[RSCamCtrl] launching reading loop dual");
                        readingLoopDual.Run();
                    }
                    else if (cameraIsActive[0])
                    {
                        System.Diagnostics.Debug.WriteLine("[RSCamCtrl] launching reading loop dual");
                        readingLoopSingle.Run();
                    }
                }
                else
            ...
            if (readingLoopDual != null)
            {
                if (readingLoopDual.errorEncountered)
                {
                    writeToEventLog(readingLoopDual.errorMessage);
                    readingLoopDual.resetAndRestartCameras();
                    errorMessageFromLoop = readingLoopDual.errorMessage;
                }
            }

            if (readingLoopSingle != null)
            {
                if (readingLoopSingle.errorEncountered)
                {
                    writeToEventLog(readingLoopSingle.errorMessage);
                    readingLoopSingle.resetAndRestartCameras();
                    errorMessageFromLoop = readingLoopDual.errorMessage;
                }
            }
            ...
            fpsReport += "last error: " + errorMessageFromLoop;
            // *************************************************************************************
        ...
        public void haltCameraControl()
        {
            ...
            if (readingLoopDual != null)
            {
                ...
                readingLoopDual.CaptureReady01 -= ReadingLoop_CaptureReady01;
                readingLoopDual.CaptureReady02 -= ReadingLoop_CaptureReady02;
                readingLoopDual.Dispose();
            }
            if (readingLoopSingle!=null)
            {
                readingLoopSingle.CaptureReady01 -= ReadingLoop_CaptureReady;
                readingLoopSingle.Failed -= ReadingLoop_Failed;
                readingLoopSingle.Dispose();
            }
